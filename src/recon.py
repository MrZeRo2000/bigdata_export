...
local_names = [re.search("\'name\': \'(\S+)\'", line)[1] for line in lines if re.search('rows:(\d+)', line) is not None]
local_rows = [re.search('rows:(\d+)', line)[1] for line in lines if re.search('rows:(\d+)', line) is not None]
